...
public class CardDAO extends AbstractSearcherDAO implements ICardDAO {
    ...
    public List<Integer> searchCardIds(FieldSearchFilter[] filters) {
        ...
        return ids.stream().map(s -> Integer.parseInt(s)).collect(Collectors.toList());
    }
	...
	public Card loadCard(Integer id) {
		...
		try {
			...
			if (res.next()) {
                ...
                if (null != date) {
                    card.setDate(new java.util.Date(date.getTime()));
                }
				card.setNote(res.getString(5));
			}
		} catch (Throwable t) {
			_logger.error("Error loading card by id {}", id,  t);
			throw new EntRuntimeException("Error loading card by id " + id, t);
		} finally {
	...
	public void addCard(Card card) {
		...
		try {
            ...
            card.setId(this.extractNextId(conn));
			//id, holder, description, date, note
			...
			throw new EntRuntimeException("Error adding new card", t);
		} finally {
	...
	public void updateCard(Card card) {
		...
		try {
			...
			throw new EntRuntimeException("Error updating a card", t);
		} finally {
			closeDaoResources(null, stat, conn);
		}
	}
	...
	public void deleteCard(Integer id) {
		...
		try {
			...
			throw new EntRuntimeException("Error deleting a card", t);
		} finally {
			closeDaoResources(null, stat, conn);
		}
	}
    ...
    protected int extractNextId(Connection conn) {
		...
		try {
			...
			throw new EntRuntimeException("Error extracting next id", t);
		} finally {
